        ...
        if (mounted) {
          ...
          setStatusHistory((prev) => {
            ...
            res.services.forEach((s: any) => {
              const prevHistory = next[s.id] ?? [];
              const last = prevHistory[prevHistory.length -1];

              if (last !== s.status) {
                next[s.id] = [...prevHistory, s.status].slice(-10);
              } else {
                next[s.id] = prevHistory;
              }
            });
  ...
  return (
    <div className="dashboard">
      ...
      <section className="panel services-panel">
        ...
        <div key={page} className="table-transition enter enter-active">
          <ServicesTable
